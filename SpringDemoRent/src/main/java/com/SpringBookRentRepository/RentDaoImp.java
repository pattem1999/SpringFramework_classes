...
public class RentDaoImp implements RentDao {
	...
	/*
	 * public Rent searchByType(String type) { // TODO Auto-generated method stub
	 * String sql="select * from rent"; //String sql="Select * from Book"; Object[]
	 * args= {type};
	 * 
	 * @SuppressWarnings("deprecation") Rent nb=jdbc.queryForObject(sql,new
	 * RowMapper<Rent>() {
	 * 
	 * @Override public Rent mapRow(ResultSet rs, int r) throws SQLException {
	 * //Book book=new Book(); Rent re=new Rent(); re.setName(rs.getString("name"));
	 * re.setCategory(rs.getString("category")); re.setPid(rs.getInt("pid"));
	 * re.setType(rs.getString("type")); re.setArea(rs.getString("area"));
	 ...
	 * re.setBedrooms(rs.getString("bedrooms")); re.setCity(rs.getString("city"));
	 * book.setBookname(rs.getString(2)); book.setCategory(rs.getString(3));
	 * book.setPublisher(rs.getString(4)); book.setPrice(rs.getInt(5)); return re; }
	 * },type); return nb; }
	 */
	public void update() {
	...
	public List<Rent> searchRentRange(double x, double y) {
		String sql="select name from rent where rentexp bwetween 1000 and 2000";
		
		//String rname=jdbc.queryForObject(sql, null);
		return null;
	...
	public void searchById(int id) {
		...
		String sql = "select * from rent where pid=?";
